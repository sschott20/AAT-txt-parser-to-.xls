    ...
    for i in range(1, len(trials)):
        ...
        if trials[i] == "Level: 3":
            for j in range(i, len(trials)):
                ...
                if trials[j].startswith("StimulusContent:") or trials[j].startswith("CorrectResponse:") or trials[j].startswith("RT:") or trials[j].startswith("ErrorCount:"):
...
# part 2 writing to excel
    ...
    # collumn titles 
    sheet1.write(0, 0, "SmokingPull", boldFont)
    sheet1.write(0, 2, "SmokingPush", boldFont)
    sheet1.write(0, 4, "NeutralPull", boldFont)
    sheet1.write(0, 6, "NeutralPush", boldFont)

    for i in range(8):
        sheet1.write(1, i, "RT" if i % 2 == 0 else "error", boldFont)
    ...
    for trial in trials: 
        ...
        sheet1.write(bottomRow[rowInt], col, int(trial[7]), regFont)
        sheet1.write(bottomRow[rowInt], col + 1, int(trial[1]), regFont)
